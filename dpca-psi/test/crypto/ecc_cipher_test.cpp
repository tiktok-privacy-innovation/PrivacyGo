...
TEST_F(EccCipherTest, serialization) {
    for (std::size_t i = 0; i < test_iter_num_; ++i) {
        ...
        ByteVector serialized_point(kEccPointLen);
        for (std::size_t idx = 0; idx < kEccPointLen; ++idx) {
            serialized_point[idx] = Byte(point_str[idx]);
        }
...
TEST_F(EccCipherTest, serialization_length) {
    ...
    for (std::size_t i = 0; i < bench_iter_num_; ++i) {
        ...
        auto ret = EC_POINT_point2oct(
                group_.get(), point.get(), POINT_CONVERSION_COMPRESSED, point_str.data(), kEccPointLen, bn_ctx_.get());
        ByteVector serialized_point(point_str.size());
        for (std::size_t idx = 0; idx < point_str.size(); ++idx) {
            serialized_point[idx] = Byte(point_str[idx]);
        }
...
TEST_F(EccCipherTest, hash_encrypt) {
    std::string email1 = "test1@tiktok.com";
    std::string phone1 = "18818881888";

    std::string email2 = "test2@tiktok.com";
    std::string phone2 = "18818882888";

    EccCipher cipher(curve_id_, 2);
    for (std::size_t i = 0; i < test_iter_num_; ++i) {
        ByteVector encrypted_email0 = cipher.hash_encrypt(email1, 0);
        ByteVector encrypted_email1 = cipher.hash_encrypt(email1, 0);
        ByteVector encrypted_email2 = cipher.hash_encrypt(email2, 0);
        ByteVector encrypted_email3 = cipher.hash_encrypt(email1, 1);

        ByteVector encrypted_phone0 = cipher.hash_encrypt(phone1, 0);
        ByteVector encrypted_phone1 = cipher.hash_encrypt(phone1, 0);
        ByteVector encrypted_phone2 = cipher.hash_encrypt(phone2, 0);
        ByteVector encrypted_phone3 = cipher.hash_encrypt(phone1, 1);
...
TEST_F(EccCipherTest, encrypt) {
    ...
    ByteVector encrypted_email0 = cipher1.hash_encrypt(email1, 0);
    ByteVector encrypted_email1 = cipher1.hash_encrypt(email1, 0);
    ByteVector encrypted_email2 = cipher1.hash_encrypt(email2, 0);
    ByteVector encrypted_email3 = cipher1.hash_encrypt(email1, 1);

    ByteVector encrypted_phone0 = cipher1.hash_encrypt(phone1, 0);
    ByteVector encrypted_phone1 = cipher1.hash_encrypt(phone1, 0);
    ByteVector encrypted_phone2 = cipher1.hash_encrypt(phone2, 0);
    ByteVector encrypted_phone3 = cipher1.hash_encrypt(phone1, 1);

    for (std::size_t i = 0; i < test_iter_num_; ++i) {
        ByteVector re_encrypted_email0 = cipher2.encrypt(encrypted_email0, 0);
        ByteVector re_encrypted_email1 = cipher2.encrypt(encrypted_email1, 0);
        ByteVector re_encrypted_email2 = cipher2.encrypt(encrypted_email2, 0);
        ByteVector re_encrypted_email3 = cipher2.encrypt(encrypted_email0, 1);

        ByteVector re_encrypted_phone0 = cipher2.encrypt(encrypted_phone0, 0);
        ByteVector re_encrypted_phone1 = cipher2.encrypt(encrypted_phone1, 0);
        ByteVector re_encrypted_phone2 = cipher2.encrypt(encrypted_phone2, 0);
        ByteVector re_encrypted_phone3 = cipher2.encrypt(encrypted_phone0, 1);
...
TEST_F(EccCipherTest, encrypt_and_div) {
    ...
    ByteVector encrypted_email0 = cipher1.hash_encrypt(email1, 0);
    ByteVector encrypted_email1 = cipher1.hash_encrypt(email1, 1);

    for (std::size_t i = 0; i < test_iter_num_; ++i) {
        ByteVector re_encrypted_email1 = cipher1.encrypt_and_div(encrypted_email0, 1, 0);
        ASSERT_EQ(encrypted_email1, re_encrypted_email1);
    }
}

TEST_F(EccCipherTest, diffie_hellman) {
    std::string sender_email1 = "test1@tiktok.com";
    std::string sender_phone1 = "18818881888";

    std::string sender_email2 = "test2@tiktok.com";
    std::string sender_phone2 = "18818882888";

    std::string receiver_email1 = "test1@tiktok.com";
    ...
    for (std::size_t i = 0; i < test_iter_num_; ++i) {
        ByteVector sender_encrypted_email1 = sender.hash_encrypt(sender_email1, 0);
        ByteVector sender_encrypted_email2 = sender.hash_encrypt(sender_email2, 0);
        ByteVector sender_encrypted_phone1 = sender.hash_encrypt(sender_phone1, 0);
        ByteVector sender_encrypted_phone2 = sender.hash_encrypt(sender_phone2, 0);

        ByteVector exchanged_sender_encrypted_email1 = receiver.encrypt(sender_encrypted_email1, 0);
        ByteVector exchanged_sender_encrypted_email2 = receiver.encrypt(sender_encrypted_email2, 0);
        ByteVector exchanged_sender_encrypted_phone1 = receiver.encrypt(sender_encrypted_phone1, 0);
        ByteVector exchanged_sender_encrypted_phone2 = receiver.encrypt(sender_encrypted_phone2, 0);

        ByteVector receiver_encrypted_email1 = receiver.hash_encrypt(receiver_email1, 0);
        ByteVector receiver_encrypted_phone1 = receiver.hash_encrypt(receiver_phone1, 0);

        ByteVector exchanged_receiver_encrypted_email1 = sender.encrypt(receiver_encrypted_email1, 0);
        ByteVector exchanged_receiver_encrypted_phone1 = sender.encrypt(receiver_encrypted_phone1, 0);
...
TEST_F(EccCipherTest, bench_encrypt) {
    std::string email1 = "test1@tiktok.com";
    EccCipher cipher(curve_id_, 2);
    ByteVector encrypted_email = cipher.hash_encrypt(email1, 0);
    for (std::size_t i = 0; i < bench_iter_num_; ++i) {
        cipher.encrypt(encrypted_email, 0);
    }
}

TEST_F(EccCipherTest, bench_encrypt_multi_threads) {
    std::string email1 = "test1@tiktok.com";
    EccCipher cipher(curve_id_, 2);
    ByteVector encrypted_email = cipher.hash_encrypt(email1, 0);
...
TEST_F(EccCipherTest, bench_hash_encrypt) {
    std::string email1 = "test1@tiktok.com";
    EccCipher cipher(curve_id_, 2);
    for (std::size_t i = 0; i < bench_iter_num_; ++i) {
        ByteVector encrypted_email = cipher.hash_encrypt(email1, 0);
...
TEST_F(EccCipherTest, bench_hash_encrypt_multi_threads) {
    std::string email1 = "test1@tiktok.com";
    EccCipher cipher(curve_id_, 2);
#pragma omp parallel for num_threads(4)
    for (std::size_t i = 0; i < bench_iter_num_; ++i) {
...
TEST_F(EccCipherTest, bench_encrypt_and_div) {
    std::string email1 = "test1@tiktok.com";
    EccCipher cipher(curve_id_, 2);
    ByteVector encrypted_email = cipher.hash_encrypt(email1, 0);
    for (std::size_t i = 0; i < bench_iter_num_; ++i) {
        cipher.encrypt_and_div(encrypted_email, 0, 1);
    }
}

TEST_F(EccCipherTest, bench_encrypt_and_div_multi_threads) {
    std::string email1 = "test1@tiktok.com";
    EccCipher cipher(curve_id_, 2);
    ByteVector encrypted_email = cipher.hash_encrypt(email1, 0);
#pragma omp parallel for num_threads(4)
