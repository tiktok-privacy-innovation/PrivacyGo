...
void dpca_psi_example(const std::string& config_path, const std::string& log_path, bool use_random_data,
    ...
    if (use_random_data) {
        ...
        if (key_size == 3) {
            ...
            for (std::size_t key_idx = 0; key_idx < key_size; ++key_idx) {
                ...
                auto unique_keys = privacy_go::dpca_psi::random_keys(
                        unique_prng, data_size - cur_intersection_size, unique_suffix);
                unique_keys.insert(
        ...
        for (std::size_t i = 0; i < feature_size; ++i) {
            features.emplace_back(privacy_go::dpca_psi::random_features(unique_prng, data_size, false));
        }
        if (!is_sender) {
            if (feature_size == 0) {
                expected_sum = 0;
        ...
        std::size_t key_size = params["common"]["ids_num"];
        privacy_go::dpca_psi::CsvFileIO csv;
    ...
    // 4. run dpca-psi.
    privacy_go::dpca_psi::DPCardinalityPSI psi;
    psi.init(params, net);
    ...
    std::uint64_t remote_shares_sum = 0;
    if (is_sender) {
        expected_sum = net->recv_value<std::uint64_t>();
        remote_shares_sum = net->recv_value<std::uint64_t>();
        net->send_value<std::uint64_t>(shares_sum);
    } else {
        net->send_value(expected_sum);
        net->send_value<std::uint64_t>(shares_sum);
        remote_shares_sum = net->recv_value<std::uint64_t>();
    }
    std::uint64_t actual_sum = shares_sum + remote_shares_sum;
...
int main(int argc, char* argv[]) {
    ...
    dpca_psi_example(FLAGS_config_path, FLAGS_log_path, FLAGS_use_random_data, FLAGS_intersection_size,
