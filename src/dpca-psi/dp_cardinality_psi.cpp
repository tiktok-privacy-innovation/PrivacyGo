...
void DPCardinalityPSI::init(const json& params, std::shared_ptr<IOChannel> net) {
    ...
    if (is_sender_) {
        ...
        io_->recv_string(sender_pk);
        LOG_IF(INFO, verbose_) << "receiver received paillier pk";

        io_->send_value<bool>(enable_djn);
        io_->send_string(receiver_paillier_.export_pk());
...
std::vector<std::vector<std::uint64_t>> DPCardinalityPSI::process() {
    auto encrypted_keys = shuffle_and_encrypt_keys_round_one();
    ...
    exchange_encrypted_keys(encrypted_keys, key_size_, received_data_size, exchanged_keys_);
    ...
    auto reshuffled_keys = reshuffle_and_encrypt_exchanged_keys_round_one();
    ...
    std::vector<std::string> single_encrypted_keys;
    exchange_single_encrypted_keys(reshuffled_keys, received_data_size, single_encrypted_keys);
    ...
    auto encrypted_features = shuffle_and_encrypt_features();
    LOG_IF(INFO, verbose_) << "shuffle and encrypt features done.";

    std::vector<std::vector<std::string>> exchanged_encrypted_features;
    ...
    auto intersection_features = filter_intersection_features(exchanged_encrypted_features, intersection_size);
    ...
    auto random_r =
            generate_additive_shares((is_sender_ ? receiver_paillier_ : sender_paillier_), intersection_features);
    LOG_IF(INFO, verbose_) << "generate additive shares done.";

    std::vector<std::vector<std::string>> exchanged_shares;
    ...
    auto shares = decrypt_and_reveal_shares(exchanged_shares, random_r, intersection_size);
...
std::vector<std::vector<std::string>> DPCardinalityPSI::shuffle_and_encrypt_keys_round_one() {
    std::vector<std::vector<std::string>> encrypted_keys;
    encrypted_keys.reserve(plaintext_keys_.size());
    for (std::size_t key_idx = 0; key_idx < key_size_; ++key_idx) {
        std::vector<std::string> encrypted_keys_i;
        encrypted_keys_i.reserve(plaintext_keys_[key_idx].size());
        if (is_sender_) {
        ...
        for (std::size_t item_idx = 0; item_idx < plaintext_keys_[key_idx].size(); ++item_idx) {
            encrypted_keys_i.emplace_back(ecc_cipher_->hash_encrypt(plaintext_keys_[key_idx][item_idx], 0));
        }
        encrypted_keys.emplace_back(encrypted_keys_i);
    }
    return std::move(encrypted_keys);
}

std::vector<std::string> DPCardinalityPSI::reshuffle_and_encrypt_exchanged_keys_round_one() {
    for (std::size_t item_idx = 0; item_idx < exchanged_keys_[0].size(); ++item_idx) {
        exchanged_keys_[0][item_idx] = ecc_cipher_->encrypt(exchanged_keys_[0][item_idx], 0);
    }

    std::vector<std::string> shuffled_keys;
    shuffled_keys.assign(exchanged_keys_[0].begin(), exchanged_keys_[0].end());
    if (is_sender_) {
        permute_and_undo(receiver_permutation_, true, shuffled_keys);
    } else {
        permute_and_undo(sender_permutation_, true, shuffled_keys);
    }
    return std::move(shuffled_keys);
}
...
std::size_t DPCardinalityPSI::repeatedly_match(std::size_t intersection_round_one) {
    ...
    for (std::size_t key_idx = 1; key_idx < key_size_; ++key_idx) {
        std::vector<std::string> filtered_exchanged_keys_i;
        ...
        exchange_single_encrypted_keys(filtered_exchanged_keys_i, received_data_size, single_encrypted_keys);
        io_->sync();
        ...
        for (std::size_t item_idx = 0; item_idx < single_encrypted_keys.size(); ++item_idx) {
            single_encrypted_keys[item_idx] = ecc_cipher_->encrypt_and_div(single_encrypted_keys[item_idx], key_idx, 0);
        }
        ...
        exchange_single_encrypted_keys(single_encrypted_keys, received_data_size, filtered_exchanged_keys_i);
        io_->sync();
...
std::size_t DPCardinalityPSI::calculate_intersection_round_one(
    ...
    for (auto flag : intersection_indices_) {
        flag.first = false;
        flag.second = std::string();
    }

    std::size_t count = 0;
    std::set<std::string> encrypted_keys_set;

    for (std::size_t item_idx = 0; item_idx < encrypted_keys.size(); ++item_idx) {
        encrypted_keys_set.insert(encrypted_keys[item_idx]);
    }
    for (std::size_t item_idx = 0; item_idx < exchanged_keys.size(); ++item_idx) {
        if (!intersection_indices_[item_idx].first) {
            auto iter = encrypted_keys_set.find(exchanged_keys[item_idx]);
            if (iter != encrypted_keys_set.end()) {
...
std::size_t DPCardinalityPSI::calculate_intersection_round_i(const std::vector<std::string>& encrypted_keys,
...
std::vector<std::vector<std::string>> DPCardinalityPSI::shuffle_and_encrypt_features() {
    std::vector<std::vector<std::string>> encrypted_features;
    ...
    auto compute_paillier_cipher_with_packing = [this, &encrypted_features, feature_size, data_size, packing_capacity,
        ...
        BigNumber bn_slot = ipcl_bn_lshift(BigNumber::One(), slot_bits_);
        for (std::size_t feat_idx = 0; feat_idx < feature_size; ++feat_idx) {
            std::vector<BigNumber> plaintexts_bn;
            plaintexts_bn.reserve(data_size);
    ...
    return std::move(encrypted_features);
}

std::vector<std::vector<std::string>> DPCardinalityPSI::filter_intersection_features(
        const std::vector<std::vector<std::string>>& encrypted_features, std::size_t intersection_size) {
    std::vector<std::pair<std::string, std::size_t>> intersection_keys;
    std::vector<std::vector<std::string>> intersection_features;
    ...
    std::vector<std::string> intersection_features_buffer;
    ...
    return std::move(intersection_features);
}
...
std::vector<std::vector<BigNumber>> DPCardinalityPSI::generate_additive_shares(
    ...
    BigNumber two_power_l = ipcl_bn_lshift(BigNumber::One(), kValueBits);
    ...
    BigNumber mask_minus_l =
            apply_packing_ ? ipcl_bn_lshift(BigNumber::One(), (slot_bits_ - 1)) - two_power_l : n_minus_l;
    ...
    return std::move(random_r);
}

std::vector<std::vector<std::uint64_t>> DPCardinalityPSI::decrypt_and_reveal_shares(
    ...
    BigNumber modulus = ipcl_bn_lshift(BigNumber::One(), kValueBits);
    ...
    auto compute_a_with_packing = [&shares, &intersection_size, &random_r, &modulus](std::size_t feature_size,
        ...
        BigNumber slot_modulus = ipcl_bn_lshift(BigNumber::One(), slot_bits);
        for (std::size_t feat_idx = 0; feat_idx < feature_size; ++feat_idx) {
            std::size_t cur_packed_num = std::min(packing_capacity, raw_feature_size - feat_idx * packing_capacity);
            for (std::size_t item_idx = 0; item_idx < intersection_size; ++item_idx) {
...
void DPCardinalityPSI::exchange_encrypted_keys(const std::vector<std::vector<std::string>>& encrypted_keys,
        std::size_t key_size, std::size_t received_data_size, std::vector<std::vector<std::string>>& received_keys) {
    std::size_t self_data_size = encrypted_keys[0].size();
    if (is_sender_) {
        std::string encrypted_keys_buffer;
        encrypted_keys_buffer.reserve(self_data_size * kEccPointLen);
        for (std::size_t key_idx = 0; key_idx < key_size; ++key_idx) {
            for (const auto& key : encrypted_keys[key_idx]) {
                encrypted_keys_buffer += key;
            }
            io_->send_string(encrypted_keys_buffer);
        ...
        std::string received_keys_buffer;
        received_keys_buffer.reserve(received_data_size * kEccPointLen);

        std::vector<std::string> received_keys_i;
        received_keys_i.reserve(received_data_size);

        for (std::size_t key_idx = 0; key_idx < key_size; ++key_idx) {
            io_->recv_string(received_keys_buffer);
            for (std::size_t item_idx = 0; item_idx < received_data_size; ++item_idx) {
                received_keys_i.emplace_back(received_keys_buffer.data() + item_idx * kEccPointLen, kEccPointLen);
            }
...
void DPCardinalityPSI::exchange_single_encrypted_keys(const std::vector<std::string>& encrypted_keys,
        std::size_t received_data_size, std::vector<std::string>& received_keys) {
    std::size_t self_data_size = encrypted_keys[0].size();
    if (is_sender_) {
        std::string encrypted_keys_buffer;
        encrypted_keys_buffer.reserve(self_data_size * kEccPointLen);
        ...
        io_->recv_string(received_keys_buffer);
        for (std::size_t item_idx = 0; item_idx < received_data_size; ++item_idx) {
            received_keys.emplace_back(received_keys_buffer.data() + item_idx * kEccPointLen, kEccPointLen);
        }
        received_keys_buffer.clear();
...
void DPCardinalityPSI::exchange_encrypted_features(const std::vector<std::vector<std::string>>& encrypted_features,
    ...
    if (is_sender_) {
        std::string encrypted_features_buffer;
        ...
        for (std::size_t feat_idx = 0; feat_idx < self_feature_size; ++feat_idx) {
            for (const auto& feature : encrypted_features[feat_idx]) {
                encrypted_features_buffer += feature;
            }
            io_->send_string(encrypted_features_buffer);
            encrypted_features_buffer.clear();
        }

        received_features.reserve(received_feature_size);

        std::string received_features_buffer;
        received_features_buffer.reserve(received_data_size * remote_paillier_len);

        std::vector<std::string> received_features_i;
        received_features_i.reserve(received_data_size);

        for (std::size_t feat_idx = 0; feat_idx < received_feature_size; ++feat_idx) {
            io_->recv_string(received_features_buffer);
            for (std::size_t item_idx = 0; item_idx < received_data_size; ++item_idx) {
                received_features_i.emplace_back(
                        received_features_buffer.data() + item_idx * remote_paillier_len, remote_paillier_len);
            }
