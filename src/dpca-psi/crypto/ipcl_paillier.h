...
public:
    ...
    // Deserializes pk.
    void import_pk(const std::string& in, bool enable_djn);
    ...
    // Encodes the BigNumber bn into string and padding zero if the length of bn is not same with n or n^2.
    // Returns serialized cipher.
    std::string encode(const BigNumber& bn, bool is_n_square) const;

    // Returns deserialized BigNumber.
    static BigNumber decode(const std::string& in);
...
private:
    // Padding zero to the end of input string.
    std::string padding_zero(const std::string& in, bool is_n_square) const;
