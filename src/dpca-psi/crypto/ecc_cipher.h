...
public:
    ...
    std::string hash_encrypt(const std::string& plaintext, std::size_t key_index);

    // Deserializes the points and exponentiates the point to `private_keys_[key_index]` power.
    // Returns a compressed form of the result point.
    std::string encrypt(const std::string& point, std::size_t key_index);

    // Deserializes the points and exponentiates the point to `private_key_[key_index_first] /
    // private_key_[key_index_second] ` power.
    // Returns a compressed form of the result point.
    std::string encrypt_and_div(const std::string& point, std::size_t key_index_first, std::size_t key_index_second);
...
private:
    ...
    // Serializes a point to a string in compressed form.
    std::string export_to_string(const ECPointPtr& point) const;

    // Deserializes a string to a point.
    ECPointPtr import_from_string(const std::string& plaintext) const;

    // Serializes a bignum to a string.
    std::string bn_to_string(const BignumPtr& bn) const;

    // SHA3-256 implementation based on openssl.
    std::string sha3_256_hash(const std::string& plaintext);
    ...
    BignumPtr ranom_oracle(const std::string& plaintext, const BignumPtr& max_value);

    // Generates strong random private key.
    void generate_private_key();

    // Computes y^2 = x^3 + a*x + b.
    BignumPtr compute_y_square(const BignumPtr& x);

    // Checks whether m is a quadratic residue modulo p.
    // Gcd(m, p) = 1, x^2 = m mod p has a solution if m^((p-1)/2) = 1 mod p.
    bool is_square(const BignumPtr& m);

    // BN_CTX used for storing temporary variables to avoid repeated memory allocation.
    const BnCtxPtr bn_ctx_;
    ...
    // Evp context used for sha3-256.
    const EvpMdCtxPtr evp_md_ctx_;
