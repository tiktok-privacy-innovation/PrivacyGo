...
TEST_F(IpclPaillierTest, test_invalid_sk) {
    ...
    EXPECT_THROW(pai.import_sk(""), std::logic_error);
    EXPECT_THROW(pai_.import_sk("123"), std::logic_error);
    EXPECT_THROW(pai_.import_sk("\x08\x02\x0f"), std::logic_error);
}

TEST_F(IpclPaillierTest, test_invalid_pk) {
    IpclPaillier pai;
    EXPECT_THROW(pai.export_pk(), std::logic_error);
    EXPECT_THROW(pai.import_pk("", true), std::logic_error);
    EXPECT_THROW(pai_.import_pk("\x03\x01", true), std::logic_error);
}
...
TEST_F(IpclPaillierTest, bn_string_conversion) {
    for (std::size_t i = 0; i < test_iter_num_; ++i) {
        for (std::size_t j = 0; j < bits_vec_.size(); ++j) {
            ...
            std::string serialized_str = ipcl_bn_2_string(bn);
            BigNumber deserialized_bn = ipcl_string_2_bn(serialized_str);
...
TEST_F(IpclPaillierTest, bn_lshift) {
    for (std::size_t i = 0; i < test_iter_num_; ++i) {
        for (std::size_t j = 0; j < bits_vec_.size(); ++j) {
            BigNumber bn = ipcl_bn_lshift(BigNumber(0x1), bits_vec_[j]);
        }
    }
}
